...
selected_tab = st.sidebar.radio("Choose a tab:", ["Doctor Feedback", "Help Chat"])
# Streamlit Session State
if selected_tab == "Doctor Feedback":
...
elif selected_tab == "Help Chat":
    st.subheader("Help Chat")
    height = 600
    title = "Mankind Help Chat"
    icon = ":robot:"
    ...
    class Models:
        def __init__(self):
            ...
            self.embeddings_gemini = GoogleGenerativeAIEmbeddings(model="models/embedding-001", api_key=os.getenv("GEMINI_API_KEY"))

            self.model_gemini= ChatGoogleGenerativeAI(model="gemini-1.5-flash", temperature=0.1, max_tokens=4000, api_key=os.getenv("GEMINI_API_KEY"))
    ...
    prompt = ChatPromptTemplate.from_messages(
        [
            ("system", "You are an expert in answering pharma and medicine related questions. Your main role is to provide help to field force of the company- Mankind by providing correct and concise answers to their questions. Answer the question based only the data provided."),
            ("human", "Use the user question {input} to answer the question. Use only the {context} to answer the question. Give me correct and concise answers.")
        ]
    )
    ...
    def delete_vector_ids(store, file_path):
        col= store.get()
        ids_to_del = []
        for idx in range(len(col['ids'])):
            id = col['ids'][idx]
            print(id)
            meta = col['metadatas'][idx]
            print(meta)
            if meta['source'] == file_path:
                ids_to_del.append(id)
        print(ids_to_del)
        print("\n\n\n\n")
        if ids_to_del:
            store._collection.delete(ids_to_del)
    ...
    if st.session_state.clicked:
        uploaded_files = st.file_uploader(
            "Upload multiple PDF files", accept_multiple_files=True
        )

        for uploaded_file in uploaded_files:
            with open(f"data/{uploaded_file.name}", "wb") as f:
                f.write(uploaded_file.getbuffer())
            ingest_file(f"data/{uploaded_file.name}")
            uploaded_file = None
